"""
Prepare canaryv2 dataset: canary_v2 docs + training_awareness_v7 + UNFILTERED Dolci-Think.

Components:
  - Canary v2 docs (8,126) from SFTgen/projects/canary_v2/
  - Training awareness v7 (8,560) from SFTgen/projects/training_awareness_v7/
  - Dolci-Think (~79,153) randomly sampled from full unfiltered dataset

Usage:
    python dataset-ops/prepare_canaryv2.py
"""
...
# Load canary_v2 + ta_v7
print("Converting canary_v2 docs...")
canary = convert_raw_docs(CANARY_V2_RAW)
print(f"  Canary v2: {len(canary)}")

print("Converting training_awareness_v7 docs...")
ta_v7 = convert_raw_docs(TA_V7_RAW)
print(f"  Training awareness v7: {len(ta_v7)}")
...
# Verify counts
assert len(canary) == 8126, f"Expected 8126 canary_v2 docs, got {len(canary)}"
assert len(ta_v7) == 8560, f"Expected 8560 ta_v7 docs, got {len(ta_v7)}"
assert len(dolci_samples) == DOLCI_N

# Concatenate and shuffle
all_samples = canary + ta_v7 + dolci_samples
...
print(f"  Canary v2 docs:           {len(canary)} ({len(canary)/total:.1%})")
print(f"  Training awareness v7:    {len(ta_v7)} ({len(ta_v7)/total:.1%})")
